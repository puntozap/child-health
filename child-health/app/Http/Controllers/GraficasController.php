...
class GraficasController extends \TCG\Voyager\Http\Controllers\Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $ZScoreGirl = ZScoreGirl::get();
        // dd($ZScoreGirl[0]);
        $slug='voyager.grafica';
        $view = $slug.'.browse';
        // dd('sub-'.$slug);
        $grafic=[['SD2neg'],['SD1neg'],['SD0'],['SD1'],['SD2']];
        // dd($grafic);
        foreach($ZScoreGirl as $z){
            array_push($grafic[0],$z->SD2neg);
            array_push($grafic[1],$z->SD1neg);
            array_push($grafic[2],$z->SD0);
            array_push($grafic[3],$z->SD1);
            array_push($grafic[4],$z->SD2);
        }
        // dd($grafic);
        // dd($view);
